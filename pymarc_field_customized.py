...
class Field(Iterator):
    ...
    def __init__(self, tag, indicators=None, subfields=None, data=u''):
        self.controlfields_uk = ['func', 'FMT', 'repId', 'LDR', 'didId', 'ds_dateAccepted', 'ds_workType', 'ds_academicTitle', 'ds_facultyName_cs', 'ds_facultyName_en', 'ds_facultyAbbr', 'ds_finalGrade_cs', 'ds_finalGrade_en',
                                'ds_studyLevel', 'ds_studyField_cs', 'ds_studyField_en', 'ds_studyProgram_cs', 'ds_studyProgram_en','ds_departmentName_cs', 'ds_departmentName_en', 'ds_keywords_cs', 'ds_keywords_en', 'ds_work_availability', 'ds_was_exported', 'ds_publication_place']
        if indicators == None:
            indicators = []
        if subfields == None:
            subfields = []
        ...
        # <JR> - 21.7.2016 change to match some custom field in MARCxml of ÚK UK
        if (self.tag < '010' and self.tag.isdigit()) or (self.tag in self.controlfields_uk):
            self.data = data
        elif self.tag in self.controlfields_uk:
            self.data = data
        elif self.tag == 'PSP' or (not self.tag.isdigit() and (self.tag not in self.controlfields_uk)): # <JR> - 17. 10. 2016 - changed to match custom ÚK UK MARCxml data fields
    ...
    def is_control_field(self):
        ...
        # <JR> - 21. 7. 2016 - changed the if statement to match the requirements of the ÚK UK marcXML
        if (self.tag < '010' and self.tag.isdigit()):
            return True
        elif self.tag in self.controlfields_uk or self.tag == 'LDR' or self.tag == '998' or self.tag:
            return True
        return False
