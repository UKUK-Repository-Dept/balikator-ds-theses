...
import re
import requests
import pyjq # needs automake and libtools packages installed for successfull install
...
class utility(object):
    ...
    def get_solr_data(self, info_type="collection_items", collection_id=None, resource_type=None, max_rows=None, start_rows=None):
        ...
        query_url = self.config.get('dspace', 'solr_endpoint') + "/select?q="
        return_format = 'json'
    ...
    def process_solr_item_data(self, json_data):
        ...
        items_data = pyjq.all('.response.docs[] | {"handle": .\"handle\", "aleph_sysno": .\"dc.identifier.aleph\"}', json_data)    
